...
import { Globe, ExternalLink, Volume2, Mic, MicOff, Loader2, Sparkles, PhoneCall, PhoneOff } from 'lucide-react';
...
export default function LanguageHubScreen() {
  ...
  // Generate speech text for language
  const getLanguageSpeech = useCallback((lang: LanguageCourse) => {
    return `${lang.name} kursi haqida ma'lumot: ${lang.description}. Ushbu yo'nalishda ${lang.levels.join(', ')} darajalari mavjud. Ibrat Farzandlari platformasida barcha video darslar va mashg'ulotlar bepul taqdim etiladi. O'rganishni hoziroq boshlashingiz mumkin!`;
  }, []);
  ...
  const handleUserVoiceQuery = async (queryText: string) => {
    ...
    try {
      const prompt: AIMessage[] = [
        {
          role: 'system',
          content: `Sen "Muqimiy Aql Markazi" chet tillari bo'limi yo'lboshchisi Mohirasan.
Foydalanuvchi ismi: ${userName}.
Hozirda tanlangan til: ${selectedLang.name}.
Foydalanuvchining savoliga o'zbek tilida juda samimiy, dono va lo'nda (2-3 jumla) qilib javob ber. Ibrat Farzandlari orqali bepul o'rganish mumkinligini eslat.`,
        },
        {
          role: 'user',
          content: queryText,
      ...
      const response = await generateAIResponse(prompt, user, 'language');
      setIsThinking(false);
  ...
  useEffect(() => {
    const welcomeSpeech = `Assalomu alaykum, ${userName}! Chet tillarini o‘rganish bo‘limiga xush kelibsiz! O‘ng tomondan o‘zingizga kerakli tilni tanlang yoki mikrofondan savol bering, men sizga barcha ma'lumotlarni aytib beraman.`;
  ...
  const handleSelectLanguage = (lang: LanguageCourse) => {
    setSelectedLang(lang);
    const speech = getLanguageSpeech(lang);
    ...
    <div className="gradient-page relative overflow-y-auto flex flex-col min-h-screen pt-20 pb-8 select-none">
      ...
      <main className="max-w-[1520px] mx-auto w-full my-auto flex flex-col justify-center px-4 sm:px-8 py-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          {/* ════════════════════════════════════════════════════════════
               RIGHT COLUMN: LANGUAGE TILES & IBRAT FARZANDLARI ACTION
             ════════════════════════════════════════════════════════════ */}
          <div className="lg:col-span-6 bg-white p-6 sm:p-8 rounded-[36px] shadow-sm border border-[#E2E8F0] flex flex-col justify-between space-y-5 animate-fade-in-scale">
            ...
            <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-4">
              ...
              <div className="text-right">
                <span className="text-xs font-black text-[#0F766E] bg-teal-50 px-3.5 py-2 rounded-xl border border-teal-200">
                  {selectedLang.levels.length} ta daraja mavjud
                </span>
              </div>
            </div>

            {/* 6 Language Cards Grid */}
              ...
              {MOCK_LANGUAGES.map((lang) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="text-sm sm:text-base font-black mb-0.5">{lang.name}</h3>
                    <span
            ...
            {/* MAIN ACTION: IBRAT FARZANDLARI OFFICIAL LAUNCH */}
