...
import { Compass, ExternalLink, Bot, Volume2, Mic, MicOff, Loader2, Sparkles, PhoneCall, PhoneOff } from 'lucide-react';
...
export default function MigrationHubScreen() {
  ...
  // Generate speech text for country
  const getCountrySpeech = useCallback((country: MigrationCountry) => {
    return `${country.name} davlatiga qonuniy migratsiya va mehnat qilish qoidalari: Talab etiladigan til: ${country.requiredLanguage}. O'rtacha oylik ish haqi ${country.salaryRange}. Talab yuqori kasblar: ${country.inDemandJobs.slice(0, 3).join(', ')}. Faqat rasmiy Tashqi Mehnat Migratsiyasi Agentligi orqali murojaat qilishni tavsiya qilaman!`;
  }, []);
  ...
  const handleUserVoiceQuery = async (queryText: string) => {
    ...
    try {
      const prompt: AIMessage[] = [
        {
          role: 'system',
          content: `Sen "Muqimiy Aql Markazi" xorijiy migratsiya bo'limi yo'lboshchisi Mohirasan.
Foydalanuvchi ismi: ${userName}.
Hozirda tanlangan davlat: ${selectedCountry.name}.
Foydalanuvchining viza, ish va xavfsiz migratsiya haqidagi savoliga o'zbek tilida juda samimiy, dono, qonuniy va lo'nda (2-3 jumla) qilib javob ber. Noqonuniy yo'llardan ogohlantir.`,
        },
        {
          role: 'user',
          content: queryText,
      ...
      const response = await generateAIResponse(prompt, user, 'migration');
      setIsThinking(false);
  ...
  useEffect(() => {
    const welcomeSpeech = `Assalomu alaykum, ${userName}! Xavfsiz va qonuniy migratsiya bo‘limiga xush kelibsiz! O‘ng tomondan kerakli davlatni tanlang yoki mikrofondan savol bering, men sizga barcha viza va ish talablarini tushuntirib beraman.`;
  ...
  const handleSelectCountry = (country: MigrationCountry) => {
    setSelectedCountry(country);
    const speech = getCountrySpeech(country);
    ...
    <div className="gradient-page relative overflow-y-auto flex flex-col min-h-screen pt-20 pb-8 select-none">
      ...
      <main className="max-w-[1520px] mx-auto w-full my-auto flex flex-col justify-center px-4 sm:px-8 py-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          {/* ════════════════════════════════════════════════════════════
               RIGHT COLUMN: COUNTRY TILES & OFFICIAL RESOURCE ACTIONS
             ════════════════════════════════════════════════════════════ */}
          <div className="lg:col-span-6 bg-white p-6 sm:p-8 rounded-[36px] shadow-sm border border-[#E2E8F0] flex flex-col justify-between space-y-5 animate-fade-in-scale">
            ...
            <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-4">
              ...
              <div className="text-right">
                <span className="text-xs font-black text-amber-800 bg-amber-50 px-3.5 py-2 rounded-xl border border-amber-200">
                  Talab etiladigan til: {selectedCountry.requiredLanguage}
                </span>
              </div>
            </div>

            {/* Country Cards Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3.5 max-h-[320px] overflow-y-auto pr-1">
              {MOCK_MIGRATION_COUNTRIES.map((country) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 sm:p-5 rounded-2xl cursor-pointer transition-all duration-200 border flex flex-col items-center justify-center text-center shadow-2xs active:scale-[0.97] min-h-[120px] ${
                      isSelected
                        ? 'bg-[#0F172A] text-white border-[#0F172A] shadow-lg scale-[1.02] ring-2 ring-teal-400'
                        : 'bg-[#FAF8F5] text-[#1C1917] border-[#E2E8F0] hover:border-[#0F766E] hover:bg-white'
                    }`}
                  >
                    <span className="text-3xl sm:text-4xl mb-1.5">{country.flag}</span>
                    <h3 className="text-sm sm:text-base font-black mb-0.5">{country.name}</h3>
                    <span
                    ...
                    >
                      {country.salaryRange || country.requiredLanguage}
                    </span>
                  </div>
                );
              })}
            ...
            {/* MAIN ACTION: OFFICIAL MIGRATION AGENCY & AI ASSISTANT */}
            <div className="p-4 sm:p-5 bg-gradient-to-r from-[#F0FDFA] to-[#E6FFFA] rounded-2xl border-2 border-[#99F6E4] flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
              <div className="text-center sm:text-left">
                <div className="font-black text-[#0F766E] text-base sm:text-lg">
                  {selectedCountry.name} bo‘yicha qonuniy ma'lumotlar
                </div>
                <div className="text-[#134E4A] text-xs font-semibold mt-0.5">
                  Faqat rasmiy davlat portali va qonuniy yo‘llanmalar asosida
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                ...
                >
                  ...
                  <span>Migratsiya AI Konsultant</span>
                </button>
              </div>
            </div>
          </div>
