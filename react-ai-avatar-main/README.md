...
- `CoderAvatar` — a second branded character on the same contract: a young developer (knit beanie, over-ear headphones, round glasses, hoodie + circuit tee). Same API as `SquirrelAvatar` — pass `state`, and `poses` opts into hand-on-chin while thinking and a typing/reading scene while working. See [`examples/10-character-avatar-developer.tsx`](examples/10-character-avatar-developer.tsx).
